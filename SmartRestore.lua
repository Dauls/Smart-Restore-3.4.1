...
function SR_IsEating()
	...
	while (loop) do
		...
		if (buff) then
			if (string.find(buff, "INV_Misc_Fork&Knife") or string.find(buff, "INV_Misc_Food_28")) then
				return true;
			end
		else
			if(index ~= 0)then
...
function SR_IsDrinking()
	...
	while (loop) do
		local buff = GetPlayerBuffTexture(index);
		if (buff) then
			if (string.find(buff, "INV_Drink") or string.find(buff, "INV_Misc_Food_28")) then
				return true;
			end
		else
			if(index ~= 0)then
